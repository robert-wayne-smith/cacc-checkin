...
{
    /// <summary>
    /// Interaction logic for ClassAttendanceDuringDateRangeReportView.xaml
    /// </summary>
    public partial class ClassAttendanceDuringDateRangeReportView : UserControl, IReportPresenterView
    {
        ...
        public ClassAttendanceDuringDateRangeReportView()
        {
            InitializeComponent();
        }
        ...
        private FlowDocument GetFlowDocument(List<CACCCheckInDb.AttendanceWithDetail> records)
        {
            ...
            {
                reportTitle = String.Format("Class Attendance On {0}",
                    startDate.ToShortDateString());
            }
            else
            {
                reportTitle = String.Format("Class Attendance From {0} to {1}",
            ...
            else
            {
                ...
                currentRow.Cells.Add(new TableCell(new Paragraph(new Run("First Name [* leader]"))));
                ...
                currentRow.Cells[3].Style = this.FindResource("ReportTableHeader") as Style;
                ...
                foreach (IGrouping<string, CACCCheckInDb.AttendanceWithDetail> classGroup in classGroups)
                {
                    ...
                    p.Inlines.Add(new Run(String.Format(" ({0} attendees)", classGroup.Count())));
                        ...
                        .ThenBy(r => r.LastName).ThenBy(r => r.FirstName))
                    {
                        ...
                        if (!String.IsNullOrEmpty(record.ClassRole) && record.ClassRole.EndsWith(ClassRoles.Teacher))
                        {
                            currentRow.Cells.Add(new TableCell(new Paragraph(new Run("* " + record.FirstName))));
                        }
                        else
                        {
                            currentRow.Cells.Add(new TableCell(new Paragraph(new Run(record.FirstName))));
                        }
                        currentRow.Cells.Add(new TableCell(new Paragraph(new Run(record.LastName))));
